...
from .reusable_charts import get_dual_pane_chart_option
...
class InteractiveStockChart:
    ...
    def __init__(self, dates: list, series_data: dict, pane1_config: dict, pane2_config: dict, chart_backgroundColor: str = '#ffffff'):
        ...
        # 기간 텍스트와 실제 기간(년)을 매핑하는 딕셔너리
        self.PERIOD_MAPPING = {
            "1년": 1, "3년": 3, "5년": 5, "10년": 10, "전체": None
        }

    def _set_date_range(self, years=None):
        """선택된 기간(년)에 따라 st.session_state의 날짜를 설정합니다."""
    ...
    def _handle_period_selection(self):
        """st.radio의 on_change 콜백으로 호출될 함수"""
        # 'period_radio' 키로 session_state에 저장된 선택된 값을 가져옵니다.
    ...
    # --- 수정된 부분: _display_widgets 메소드를 라디오 버튼 방식으로 변경 ---
    def _display_widgets(self):
        """기간 설정을 위한 위젯들을 표시합니다."""
        with st.container(border=True):
            st.write("**차트 기간 설정**")

            # st.radio를 사용하여 기간 선택 옵션을 만듭니다.
            st.radio(
                label="**한번에 차트 기간 설정**",
                options=list(self.PERIOD_MAPPING.keys()),
                key="period_radio",  # session_state에 저장될 키
                on_change=self._handle_period_selection, # 선택이 바뀔 때마다 콜백 함수 호출
                horizontal=True, # 옵션을 가로로 나열합니다.
                index=None, # 초기에는 아무것도 선택되지 않은 상태로 둡니다.
                label_visibility="collapsed" # 레이블을 숨기는 핵심 부분입니다.
